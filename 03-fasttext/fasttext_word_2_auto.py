...
model = fasttext.train_supervised(
    input=config.process_train_datapath_word,  # 输入的训练数据文件
    autotuneValidationFile=config.process_dev_datapath_word,  # 自动调参的验证集.
    # autotuneDuration=120,  # 120秒(自动调参的时长)
    # thread=1,       # 训练线程数, 设置为1(单线程) 避免多线程的随机波动, 确保实验可复现.
    # verbose=3,      # 日志详细程度(3表示: 输出最详细信息)
    seed=40         # 随机数种子, 确保模型可复现.909
)


# todo 3. 模型保存.
model_path = config.ft_model_save_path + f'/model_word_2_auto_{current_time}.bin'
